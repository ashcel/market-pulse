...
<Suspense fallback={<ChartSkeleton />}>
  {showChart && <PriceChart data={data} />}
</Suspense>
```
...
useEffect(() => { setFiltered(filter(items, search)); }, [search]);
...
useEffect(() => { connect(socket); }, [socketConfig]);
...
for (const x of items) {
  if (x.active) { names.push(x.name); total += x.value; }
}
```
