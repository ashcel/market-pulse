...
// Extracted from the former `/trades` route body (IA-REDESIGN-2026-07-23
// §4.3 Journal merge). `OpenPositionsPanel` + `TradeHistoryPanel` are the
// Open / History tab content on `/journal`; `TradesPanel` composes both
// unchanged so `/trades` keeps resolving as a thin wrapper.
...
// ── Open tab (live positions) ─────────────────────────────────────────────────

/**
 * Live open positions — the Open tab on `/journal` and the top half of the
 * `/trades` thin wrapper. Self-contained (own auth gate) so it can be
 * mounted standalone inside a tab.
 */
...
/**
 * Closed-trade journal — the History tab on `/journal` and the bottom half
 * of the `/trades` thin wrapper. Self-contained (own auth gate).
 */
export function TradeHistoryPanel() {
  ...
  if (!authenticated) {
    return (
      <IqCard className="space-y-2 text-center text-sm text-muted-foreground">
        <p>Your trade journal lives on the server — sign in to see your closed-trade history.</p>
        <p>
          <Link to="/login" className="font-medium text-info underline-offset-2 hover:underline">
            Sign in
          </Link>{" "}
          to access your trade history.
        </p>
      </IqCard>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {historyLoading ? (
        <IqCard className="text-center text-sm text-muted-foreground py-6">Loading journal…</IqCard>
...
// ── /trades thin wrapper (Open + History composed, unchanged layout) ─────────

/**
 * Body of the `/trades` thin-wrapper route — the route file supplies the
 * page's `PageHeader` and outer `space-y-5 pb-20 lg:pb-6` spacing (matching
 * the pre-merge layout exactly); this just stacks Open + History.
 */
