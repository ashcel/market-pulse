/**
 * Data capture for the Phase 3 i.mss trigger spike (research/phase3-spike.md):
 * the 18-asset universe on every timeframe the comparison touches —
 * execution TFs 15m/1h/4h plus their intent-paired context TFs 1h/4h/1d —
 * 1000 closed bars each from Binance spot. Same fetcher shape as
 * `phase2-spike/fetch-universe-data.ts` (identical window discipline keeps
 * the two spikes comparable).
 *
 *   bun run research/scripts/phase3-spike/fetch-spike-data.ts <outDir>
 *
...
if (!outDir) {
  console.error("usage: bun fetch-spike-data.ts <outDir>");
  process.exit(1);
}
