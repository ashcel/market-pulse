...
### Dreimann — a top-down multi-timeframe _framework_

Evidence: `research/dreimann/trades.txt` + the 7 TradingView screenshots
(15m/1h/4h crypto perps). Vocabulary: _Pro Swing, Pro Internal, Counter
Internal, objective, weak structure, POI, i.mss, liquidity left/right,
premium/discount._ This is the ICT-derived "swing → internal" school
...
2. **Name the objective** = the _opposing weak high / weak low_ price is drawn
...
4. **Wait for internal alignment** — the `i.mss` label = an _internal_ market
...
| Label                        | Meaning                                                                   | Read                     |
| ---------------------------- | ------------------------------------------------------------------------- | ------------------------ |
| Pro Swing + Pro Internal     | with HTF swing _and_ current internal                                     | A+ (TRX TP3)             |
| Pro Swing + Counter Internal | with HTF swing, _against_ live internal — betting the internal flips back | higher risk (ETHFI → SL) |
...
- **ETHFI (SL):** _Counter Internal_ — with swing but against internal. The
  framework itself flags this as lower-probability.
- **ZEC (SL, "would have reached TP without stop"):** POI was respected but the
  stop sat _inside_ the POI's liquidity noise. Lesson: **stop placement is
  relative to the POI + resting liquidity, not a fixed distance.**

### Sanos — a bottom-up mechanical _entry trigger_ (iFVG model)
...
1. **Liquidity sweep** — a 5m swing high/low that _previously rejected from an
   FVG_ gets swept. Don't drop below 5m for the sweep (lower TF = lower prob).
2. **Delivery** — price is _delivering out of an FVG_ (iFVG inversion) on a
...
**Dreimann answers _where_ and _why_ (context, POI, objective). Sanos answers
_when_ (precision trigger inside a zone).** Sanos's "bonus" tier
(premium/discount + SMT) _is_ Dreimann's context. They nest naturally — but only
...
- **Which TF pair defines swing vs internal.** The notes map swing=H4,
  internal=M15 (cross-timeframe), _not_ two pivot scales on one chart. This
...
- **What makes a high "weak" vs "strong."** Standard SMC: a swing that produced
  a BOS is _strong_ (protected); one that failed to is _weak_ (target). Assumed,
  not written.
...
- **"Clear, obvious target"** is exactly our liquidity-pool map — encodable, but
  the _threshold_ for "obvious" is discretionary.
...
That is roughly the _left half_ of Dreimann and the _targets_ half of Sanos. The
gaps:
...
"clear target," and Dreimann's premium/discount _is_ Sanos's bonus tier. Best
...
1. **TF / instrument scale mismatch.** Sanos's 9-point FVG and 30s–5m delivery
   are NQ-intraday. Grafting them onto Dreimann's H4/M15 crypto _swing_ changes
...
2. **Anticipation vs confirmation conflict.** Dreimann's edge is often the
   ...
   **confirmation-only** ("no delivery → no trade"). These are two points on one
   ...
   real slice of Dreimann's setups; taking Dreimann limits _without_ a trigger
...
4. **Trigger layer** (MSS / iFVG-delivery / sweep) **separable from the POI**, so
   anticipation vs confirmation is a _policy choice per intent_, not baked in.
   Solves G6, G9, and the §4.2 conflict cleanly.

`setupType` then becomes a _derived description_ of a composed setup, not the
primary model.
...
pairing already supplies "external" structure. His swing=H4 / internal=M15 _looks_
...
rejection was made for supplying _external context_, not an _internal trigger_ —
a different requirement. **So G1's tier mechanism is unresolved (see Risk R3): run
a spike before committing to cross-TF.** What _is_ safe: **G2 (strong/weak typing)
...
- **Strong/weak swing typing (G2):** a swing high is **strong** iff a later
  swing low broke the prior swing low _after_ it (i.e., it is the origin of a
...
- **Equilibrium / premium-discount (G4):** for the active dealing range
  ...
  (the range is bounded by _strong_ swings), so sequence strength → range →
...
- **Internal MSS trigger (G9):** on the execution TF, a CHoCH of the _internal_
...
### R1 — Strong/weak typing breaks the frozen-record invariant _(hits Phase 0)_
...
function of how much history you've seen, so it _cannot_ be frozen at
...
### R3 — Cross-TF is an _approximation_ of nested structure, not an equivalent
...
- **Chosen for now:** treat as a **spike** — test on the 7 charts whether
  context-TF structure reproduces the swing pivots Dreimann drew _before_ picking.
...
### R5 — The harness _and_ the ground truth are inadequate for the new signals
...
- **Phase 0 — Instrumentation (no verdict impact, no shadow claim).**
  ...
  **reproduce the annotations on the 7 charts** (logic correctness), _not_
  expectancy and _not_ threshold tuning (R5). No `SwingPoint` schema change.
- **Phase 1 — Objectives + POI entry (merged G3 + G8).** Draw-on-liquidity
  ...
  a `SetupType` change (R2). _Reordered 2026-07-10 (owner decision): Phase 0.5
  ...
  but consumed by no decision until the post-0.5 graduation gate._ See
...
**The question is decision quality, not fidelity.** We are _not_ proving which
...
immaterial.** Nested must _earn_ adoption by clearing every gate below. This
...
_internal_ tier (and therefore the internal-MSS trigger and the
