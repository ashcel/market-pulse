...
- Needs restart: yes (route + AI-context change; not yet restarted)
- Flags for user: M0-T5 (all sub-tasks) and the two M0 success-criteria bullets it closes are done. Remaining open M0 items: `docs/score-inventory.md`'s coverage is complete but M0-T6 (deploy path) and M0-T7 (transport hardening) are still open.
